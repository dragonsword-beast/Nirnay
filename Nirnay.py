...
st.markdown(
    ...
    * {{
    ...
    .chat-navigator .stButton>button,
    .chat-navigator .stButton>div>button,
    .chat-navigator .stButton>div>div>button,
    .chat-navigator + div .stButton>button,
    .chat-navigator + div .stButton>div>button,
    .chat-navigator + div .stButton>div>div>button,
    .chat-navigator ~ div .stButton>button,
    .chat-navigator ~ div .stButton>div>button,
    .chat-navigator ~ div .stButton>div>div>button {{
        ...
        border: 1px solid rgba(34, 101, 58, 0.35) !important;
        box-shadow: 0 14px 22px rgba(16, 83, 36, 0.2) !important;
...
def back_to_chat_interface():
    set_page("chat_selection")
...
if page == "chat":
    ...
    with col2:
        st.button(
            "↩️ Back to Chat Interface",
            key=f"back_to_chat_selection_{mode}",
            on_click=back_to_chat_interface,
        )
...
if page == "analysis":
    ...
    st.markdown("<div style='margin: 0.1rem 0 0; text-align:right;'>", unsafe_allow_html=True)
    st.button("🤖Talk with AI Assistants", key="go_to_chat_selection", on_click=set_page, args=("chat_selection",))
    st.markdown("</div>", unsafe_allow_html=True)

    render_analysis_chat_styles()
    render_chat_styles()

if page == "chat_selection":
    st.markdown("<div class='topbar'><h1>🤖 Chat Interface</h1></div>", unsafe_allow_html=True)
    st.markdown(
        """
        <div class="analysis-patient-summary">
            <p>Select the chatbot you want to use for either detailed clinical assistance or a quick medical summary.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )

    st.button("⬅️ Back to Analysis", key="back_to_analysis", on_click=set_page, args=("analysis",))
...
st.markdown("<div id='clinical-interface' style='position:relative; top:-110px; padding-top:110px;'></div>", unsafe_allow_html=True)
