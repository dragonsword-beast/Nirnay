...
st.markdown(
    ...
    * {{
    ...
    .summary-card .section-row {{
        margin-top: 0.85rem;
    }}

    .summary-card .section-pill {{
    ...
    .stButton>button {{
        ...
        box-shadow: 0 14px 30px rgba(79, 209, 197, 0.22);
    }}
...
# Header
st.markdown('<h1 class="main-header">👨‍⚕️Nirnay🩺</h1>', unsafe_allow_html=True)
st.markdown('<p class="subtitle"> Your Health Companion💊. </p>', unsafe_allow_html=True)

st.sidebar.markdown('# 🟠Nirnay Hub')
st.sidebar.markdown('A refined assistant for structured medical screening and system-specific analysis."<p>🔬 Remember: This is an AI-assisted diagnostic tool. Always consult healthcare professionals for final decisions.</p><p>Created with passion by Aarko Batabyal & Saptak Bhattacharjee</p>', unsafe_allow_html=True )
st.sidebar.markdown('---')
st.sidebar.markdown('### How to use')
st.sidebar.markdown(
    '''
    1. Complete the patient profile.
    2. Agree to the disclaimer.
    3. Continue to the Analysis dashboard.
    4. Enter values only for available clinical findings.
    '''
)
st.sidebar.markdown('---')
...
st.sidebar.markdown(
    ...
    • Focus on confirmed measurements.
    • Use the analysis page for clear results.
    '''
)

# ------------ Page state / routing -------------
if hasattr(st, "experimental_get_query_params"):
    page = st.experimental_get_query_params().get("page", ["profile"])[0]
elif hasattr(st, "get_query_params"):
    page = st.get_query_params().get("page", ["profile"])[0]
else:
...
# ------------ Disclaimer -------------
with st.expander("⚠️Please read the Medical Disclaimer", expanded=False):
    st.markdown(
        """
...
if page != "analysis":
    col1, col2 = st.columns([1.2, 0.95])

    with col1:
        st.markdown(
            """
            <div class="card">
                <div class="card-header">👤 Patient Identity</div>
                <p style="margin: 0; color: rgba(229,238,247,0.85);">Enter the patient's name, age, and gender to begin the clinical workflow. This ensures your analysis is anchored to a professional intake process.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )

        st.session_state.patient_name = st.text_input("👤 Patient Full Name", value=st.session_state.patient_name, placeholder="Enter full name")
        st.session_state.patient_age = st.text_input("🎂 Age", value=st.session_state.patient_age, placeholder="Years")
    ...
    with col2:
        st.markdown(
            ...
            <div class="summary-card">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; margin-bottom: 1rem; text-color: "{accent_teal}";">
                    <div>
                        <div class="hero-badge">Clinical-grade capture</div>
                        <h2>Focused medical intake built for professional review</h2>
                    </div>
                    <div style="text-align: right; color: rgba(255,255,255,0.82); font-size: 0.88rem;">
                        Trusted workflow
                    </div>
                </div>
                <p>Collect patient details, structured findings, and confidently move into the analysis workflow with an interface designed for speed, clarity, and professional trust.</p>
                <div class="section-row">
                    <span class="section-pill">Structured patient intake</span>
                    <span class="section-pill">Clinical decision support</span>
                    <span class="section-pill">Clear analysis path</span>
                </div>
                <div class="stat-grid">
                    <div class="hero-metric">
                        <div>
                            <strong><text color="{accent_teal}">2 min</text></strong>
                            <span>Preparation for analysis</span>
                        </div>
                    </div>
                    <div class="hero-metric">
                        <div>
                            <strong><text color="{accent_teal}">Professional</text></strong>
                            <span>Clinical presentation</span>
                        </div>
                    </div>
                    <div class="hero-metric">
                        <div>
                            <strong><text color="{accent_teal}">Modern</text></strong>
                            <span>Intake experience</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    if not valid_profile:
        st.markdown(
            ...
            <div style="background: linear-gradient(135deg, {warning_yellow} 0%, {danger_red} 100%); 
                        ...
                        font-weight: 600; 
                        margin: 1rem 0;">
                ⚠️ Complete patient profiling and disclaimer agreement are required to proceed.
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        """
        <div class="footer">
            <p>Created with passion by Aarko Batabyal & Saptak Bhattacharjee</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if page == "analysis":
    ...
    st.markdown(
        ...
        <div class="analysis-banner">
            <div class="analysis-summary">
                <div class="card-header">🔬 Analysis Dashboard</div>
                <p>Review system-level clinical input with an emphasis on validated findings and concise, professional recommendations.</p>
                <ul>
                    <li>Patient details remain visible throughout the workflow.</li>
                    <li>Only populated clinical sections contribute to the final summary.</li>
                    <li>Generated results are optimized for clinical handoff.</li>
                </ul>
            </div>
            <div class="analysis-sidebar-card">
                <h3>Patient summary</h3>
                <p><strong>Name:</strong> {patient_name}</p>
                <p><strong>Age:</strong> {patient_age}</p>
                <p><strong>Gender:</strong> {patient_gender}</p>
                <p style="margin-top: 1rem; color: rgba(229,238,247,0.75);">Patient identity is retained while you enter clinical findings in the tabs below.</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if st.button("⬅️ Back to Profile"):
        if hasattr(st, "experimental_set_query_params"):
            ...
            st.set_query_params(page="profile")
        else:
    ...
    if "chat_alt_history" not in st.session_state:
        st.session_state.chat_alt_history = []
    if "chat_visible" not in st.session_state:
        st.session_state.chat_visible = False
    if "chat_alt_visible" not in st.session_state:
        st.session_state.chat_alt_visible = False

    st.markdown(
        """
        <style>
        .chat-toggle-right, .chat-toggle-left {
        ...
        .chat-label {
            ...
            border-radius: 999px;
            border: 1px solid rgba(16, 30, 50, 0.08);
        ...
        .chat-toggle-right > div,
        .chat-toggle-left > div {
        ...
        .chat-toggle-right button,
        .chat-toggle-left button {
            color: white;
            border: none;
            border-radius: 50%;
            width: 52px;
        ...
        .chat-toggle-right button:hover,
        .chat-toggle-left button:hover {
            transform: translateY(-1px);
            box-shadow: 0 14px 28px rgba(0,0,0,0.24);
            opacity: 0.97;
        }
        .chat-toggle-right button:focus,
        .chat-toggle-left button:focus {
            outline: none !important;
            box-shadow: 0 10px 24px rgba(0,0,0,0.28);
        }
        .chat-toggle-right button.green {
            background: linear-gradient(135deg, #25D366 0%, #1EA64C 100%);
        }
        .chat-toggle-left button.blue {
            background: linear-gradient(135deg, #0D6EFD 0%, #1A83FF 100%);
        }
        .chat-panel,
        .chat-panel-alt {
            position: fixed;
    ...
    st.markdown("<div class='chat-toggle-left'>", unsafe_allow_html=True)
    st.markdown("<div class='chat-label'>Medical Chat</div>", unsafe_allow_html=True)
    green_clicked = st.button("💬", key="toggle_chat_button", help="Open main medical chatbot")
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='chat-toggle-right'>", unsafe_allow_html=True)
    st.markdown("<div class='chat-label'>Quick Chat</div>", unsafe_allow_html=True)
    blue_clicked = st.button("🔷", key="toggle_chat_alt_button", help="Open quick compact chatbot")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if green_clicked:
        ...
        if st.session_state.chat_visible:
            st.session_state.chat_alt_visible = False
    if blue_clicked:
        st.session_state.chat_alt_visible = not st.session_state.chat_alt_visible
        if st.session_state.chat_alt_visible:
            st.session_state.chat_visible = False

    if st.session_state.chat_visible:
        st.markdown("<div class='chat-panel'>", unsafe_allow_html=True)
        st.markdown("<h4>Medical Chatbot</h4><p style='margin:0 0 0.5rem 0; color:#333;'>Ask Nirnay a clinical question and receive a full response.</p>", unsafe_allow_html=True)
        ...
        for msg in st.session_state.chat_history:
            if msg["role"] == "user":
                ...
                st.markdown(
                    f"<div class='chat-message chat-bot'>{msg['content']}</div>",
                    unsafe_allow_html=True,
                )

        with st.form("nirnay_chat_form"):
            user_input = st.text_input("Type your medical question...", key="nirnay_chat_prompt")
        ...
        if send_clicked:
            if user_input and user_input.strip():
                ...
                try:
                    ...
                    completion = client.chat.completions.create(
                        ...
                        messages=[
                            {"role": "system", "content": (
                                ...
                                "Provide general medical information only, not medical advice. "
                                "Always remind users to consult a qualified healthcare provider for final clinical decisions."
                            )},
                            *st.session_state.chat_history,
                        ],
                    ...
                    st.error(f"AI chatbot error: {exc}")
            else:
    ...
    if st.session_state.chat_alt_visible:
        st.markdown("<div class='chat-panel-alt'>", unsafe_allow_html=True)
        st.markdown("<h4>Quick Chat</h4><p style='margin:0 0 0.75rem 0;'>Ask a shorter question and receive a compact answer.</p>", unsafe_allow_html=True)

        for msg in st.session_state.chat_alt_history:
            if msg["role"] == "user":
                st.markdown(
                    f"<div class='chat-message chat-user'>{msg['content']}</div>",
                    unsafe_allow_html=True,
                ...
                st.markdown(
                    f"<div class='chat-message chat-bot-alt'>{msg['content']}</div>",
                    unsafe_allow_html=True,
                )

        with st.form("nirnay_chat_alt_form"):
            user_input = st.text_input("Type your compact question...", key="nirnay_chat_prompt_alt")
            send_clicked = st.form_submit_button("Send Compact")

        if send_clicked:
            if user_input and user_input.strip():
                st.session_state.chat_alt_history.append({"role": "user", "content": user_input.strip()})
                try:
                    if "GROQ_API_KEY" in st.secrets:
                        client = Groq(api_key=st.secrets["GROQ_API_KEY"])
                    else:
                    ...
                    completion = client.chat.completions.create(
                        ...
                        messages=[
                            {"role": "system", "content": (
                                "You are a medical assistant. "
                                "Answer briefly and compactly in 1-2 short sentences. "
                                "Provide general medical information only, not medical advice. "
                                "Always remind users to consult a qualified healthcare provider for final clinical decisions."
                            )},
                            *st.session_state.chat_alt_history,
                    ...
                    bot_response = bot_response or "No response received from the AI."
                    st.session_state.chat_alt_history.append({"role": "assistant", "content": bot_response})
                except Exception as exc:
                    st.error(f"AI chatbot error: {exc}")
            else:
                st.warning("Please enter a question before sending.")
...
def make_inputs(tab, fields):
    ...
    if numbers:
        for row in chunked(numbers, 6):
            cols = tab.columns(len(row))
            for col, (k, v) in zip(cols, row):
                out[k] = parse_float(col.text_input(v["label"], key=f"{v['id']}"))

    if checks:
        with tab.expander("Symptoms & clinical signs", expanded=True):
            for row in chunked(checks, 6):
                cols = tab.columns(len(row))
                for col, (k, v) in zip(cols, row):
...
for (tab_title, defs), tab_obj in zip(tab_names, tabs_objs):
    with tab_obj:
        collected[tab_title] = make_inputs(st, defs)
...
# ------------ Run Diagnosis (Auto-generated) -------------
st.markdown(
    """
    <div class="card">
        <div class="card-header">📋 NIRNAY DIAGNOSIS RESULTS</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
output = []
output.append(f">>> NIRNAY FOR: {patient_name.upper()} <<<")
output.append("-" * 50)

# Helper function to check if a section has any input data
def has_section_data(checks_dict):
    """Check if a section has any non-None or truthy values"""
    for value in checks_dict.values():
        if value is not None and value is not False and value != "":
            return True
    return False

# Check which sections have data
metabolic_has_data = has_section_data(v_checks)
cardiac_has_data = has_section_data(c_checks)
onco_has_data = has_section_data(o_checks)
neural_has_data = has_section_data(n_checks)
gynae_has_data = has_section_data(g_checks)
immuno_has_data = has_section_data(i_checks)
endo_has_data = has_section_data(endo_checks)
ped_has_data = has_section_data(ped_checks)
derm_has_data = has_section_data(derm_checks)
psych_has_data = has_section_data(psych_checks)
neph_has_data = has_section_data(neph_checks)
hem_has_data = has_section_data(hem_checks)
...
for line in output:
    if line.startswith("[CRITICAL]"):
        formatted_output.append(f"🚨 **<span style='color: #FF0000;'>{line}</span>**")
    elif line.startswith("[ALERT]"):
        formatted_output.append(f"⚠️ **<span style='color: #FFA500;'>{line}</span>**")
    elif line.startswith("[!]"):
        formatted_output.append(f"⚡ **<span style='color: #FFFF00;'>{line}</span>**")
    elif line.startswith("[+]"):
        formatted_output.append(f"✅ **<span style='color: #00FF00;'>{line}</span>**")
    elif line.startswith(">>>"):
...
st.markdown(
    ...
    <div class="result-card">
        <h2>🩺 NIRNAY DIAGNOSIS RESULTS</h2>
        {''.join(f'<span class="result-line">{line}</span>' for line in formatted_output)}
    </div>
    """,
    unsafe_allow_html=True
)
