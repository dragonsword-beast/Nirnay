...
def render_analysis_chat_styles():
    st.markdown(
        ...
        .chat-nav-panel .stButton>button {
            min-width: 220px !important;
            width: auto !important;
            border-radius: 999px !important;
            padding: 0.85rem 1.15rem !important;
            font-weight: 700 !important;
            background: linear-gradient(135deg, #4cbeb8 0%, #2549eb 100%) !important;
            color: #ffffff !important;
            border: none !important;
            box-shadow: 0 14px 30px rgba(79,209,197,0.18) !important;
        }
        .chat-nav-panel .stRadio {
            background: transparent !important;
            border: none !important;
            box-shadow: none !important;
        }
        .chat-nav-panel .stRadio label,
        .chat-nav-panel .stRadio span {
            color: #f3f7fb !important;
        }
        .chat-nav-panel .stRadio input[type="radio"] {
            accent-color: #4cbeb8 !important;
            outline: none !important;
            box-shadow: none !important;
        }
        .chat-options-grid {
        ...
        .chat-option-card .stButton>button {
            width: 100% !important;
            border-radius: 999px !important;
...
def render_chat_styles():
    st.markdown(
        ...
        .chat-close-row .stButton>button {
            width: auto !important;
            border-radius: 999px !important;
...
if page == "chat":
    ...
    st.markdown(
        "<div class='chat-close-row'>",
        unsafe_allow_html=True,
    )
    if st.button("← Back to Analysis", key=f"back_{mode}_button"):
        st.session_state.page = "analysis"
...
if page == "analysis":
    ...
    if "chat_page" not in st.session_state:
        st.session_state.chat_page = None
    if "chat_choice" not in st.session_state:
        st.session_state.chat_choice = "Medical Assistant"

    chat_choice_label = st.radio(
        "Choose Assistants",
        ["Medical Assistant 👩‍⚕️🩺", "Quick Assistant ⚡"],
        index=0 if st.session_state.chat_choice == "Medical Assistant" else 1,
        horizontal=True,
        key="chat_choice_radio",
        label_visibility="collapsed",
    )
    st.session_state.chat_choice = (
        "Medical Assistant"
        if chat_choice_label.startswith("Medical Assistant")
        else "Quick Assistant"
    )

    st.markdown("<div class='chat-nav-panel'>", unsafe_allow_html=True)
    st.markdown(f"<div class='chat-nav-label'>Choose one chatbot at a time and open it below.</div>", unsafe_allow_html=True)
    if st.button("Open selected chatbot", key="launch_selected_chat"):
        st.session_state.chat_mode = (
            "medical" if st.session_state.chat_choice == "Medical Assistant" else "quick"
        )
    ...
    st.markdown("<div class='chat-options-grid'>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(
            ...
            <div class='chat-option-card'>
                ...
                <p style='margin-top:1rem;font-size:0.92rem;color:rgba(232,239,247,0.75);'>Select "Medical Assistant" above and press the button to launch.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with col2:
        st.markdown(
            ...
            <div class='chat-option-card'>
                ...
                <p style='margin-top:1rem;font-size:0.92rem;color:rgba(232,239,247,0.75);'>Select "Quick Summary" above and press the button to launch.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
    st.markdown("</div>", unsafe_allow_html=True)
    # Inside your analysis page rendering logic
